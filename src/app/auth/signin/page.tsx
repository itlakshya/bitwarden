...
function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recaptchaToken = await executeRecaptchaAction('user_login');
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-white via-slate-50/70 to-white text-slate-900 p-4">
      <div className="mb-8 flex flex-col items-center">
        ...
        <p className="mt-2 text-sm text-slate-600">Welcome back to your workspace</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Link href="/auth/forgot-password" title="Forgot password" className="text-xs text-slate-600 hover:text-yellow-600 transition-colors">
                Forgot password?
              </Link>
            </div>
            <input
